...
func TestAccSceneXYPlan(t *testing.T) {
	b := fakebridge.New()
	defer b.Close()
	...
	for _, xy := range []struct {
		...
		steps = append(steps, resource.TestStep{Config: cfg,
			ConfigPlanChecks: resource.ConfigPlanChecks{PreApply: []plancheck.PlanCheck{
				plancheck.ExpectKnownValue("hue_scene.test", tfjsonpath.New("actions").AtMapKey(fakebridge.LightID).AtMapKey("color_xy").AtMapKey("x"), knownvalue.Float64Exact(xy.point.X)),
			}},
			Check: resource.TestCheckNoResourceAttr("hue_scene.test", "actions."+fakebridge.LightID+".color_hex"),
